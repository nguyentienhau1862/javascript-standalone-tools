					...
					for (const key2 of keyPrefixed) {
						promises.push(
							Promise.resolve()
								.then(() => store.get(key2))
								.then((data) =>
									typeof data === "string" ? this.opts.deserialize(data) : this.opts.compression ? this.opts.deserialize(data) : data
								)
								.then((data) => {
...
// src/cli/options/get-context-options.js
var detailedCliOptions = normalizeOptionSettings(cli_options_evaluate_default).map((option) => normalizeDetailedOption(option));
...
function supportInfoToContextOptions({ options: supportOptions, languages }) {
	const detailedOptions = [...detailedCliOptions, ...supportOptions.map((apiOption) => apiOptionToCliOption(apiOption))];
	return {
		supportOptions,
		languages,
		detailedOptions
...
function locatePathSync(paths, { cwd: cwd2 = process3.cwd(), type = "file", allowSymlinks = true } = {}) {
	checkType(type);
	cwd2 = toPath(cwd2);
...
function getHashOfOptions(options) {
	...
	const hash = createHash(`${prettierVersion}_${nodeVersion}_${(0, import_fast_json_stable_stringify2.default)(options)}`);
...
function getOptions(argv, detailedOptions) {
	return Object.fromEntries(detailedOptions.filter(({ forwardToApi }) => forwardToApi).map(({ forwardToApi, name }) => [forwardToApi, argv[name]]));
}
...
function createApiDetailedOptionMap(detailedOptions) {
	return Object.fromEntries(
		detailedOptions.filter((option) => option.forwardToApi && option.forwardToApi !== option.name).map((option) => [option.forwardToApi, option])
	);
}
...
async function format3(context, input, opt) {
	...
	if (context.argv.debugPrintComments) {
		return {
			formatted: await prettier.format(JSON.stringify((await prettier.formatWithCursor(input, opt)).comments || []), { parser: "json" })
		};
	}
	...
	if (context.argv.debugCheck) {
		...
		if (pp !== pppp) {
			...
			const ast = stringify4((await prettier.__debug.parse(input, opt, { massage: true })).ast);
			const past = stringify4((await prettier.__debug.parse(pp, opt, { massage: true })).ast);
			if (ast !== past) {
				...
				throw new DebugError("ast(input) !== ast(prettier(input))\n" + astDiff + "\n" + diff(input, pp));
			}
		}
	...
	if (performanceTestFlag == null ? void 0 : performanceTestFlag.debugBenchmark) {
		...
		context.logger.debug("'--debug-benchmark' measurements for formatWithCursor: " + JSON.stringify(result, null, 2));
	} else if (performanceTestFlag == null ? void 0 : performanceTestFlag.debugRepeat) {
		const repeat = performanceTestFlag.debugRepeat;
		context.logger.debug(`'${performanceTestFlag.name}' found, running formatWithCursor ${repeat} times.`);
		let totalMs = 0;
		...
		context.logger.debug(`'${performanceTestFlag.name}' measurements for formatWithCursor: ${JSON.stringify(results, null, 2)}`);
	}
...
async function formatStdin(context) {
	...
	try {
		...
		const options = await get_options_for_file_default(context, filepath ? path9.resolve(filepath) : void 0);
...
async function formatFiles(context) {
	...
	if (context.argv.cache) {
		formatResultsCache = new format_results_cache_default(cacheFilePath, context.argv.cacheStrategy || "content");
...
${error2.message}`
		...
		const isCacheExists = formatResultsCache == null ? void 0 : formatResultsCache.existsAvailableFormatResultsCache(filename, options);
		let result;
		let output;
		try {
			if (isCacheExists) {
...
async function printSupportInfo() {
	...
	const supportInfo = {
		...
		options: sortByName(options).map((option) => omit(option, ["cliName", "cliCategory", "cliDescription"]))
	};
...
function getOptionDefaultValue(context, optionName) {
	...
	return (
		...
		((_a = context.supportOptions.find((option2) => !option2.deprecated && option2.name === optionCamelName)) == null ? void 0 : _a.default)
	);
}
